...
class PASTA:
    ...
    def __call__(self, img):
        ...
        if self.mode == "prop":
            ...
            inv = np.tile(inv, (amp_src.size(1), 1, 1))
            inv += offset
            prop = np.fft.fftshift(inv, axes=[-2, -1])
            # prop = torch.tensor(prop).cpu()
            amp_src = amp_src.squeeze(0).cpu()
            random_tensor = torch.tensor(np.random.normal(np.ones(prop.shape), prop), dtype=torch.float32)
            random_tensor = random_tensor.to(amp_src.device)
            amp_src = amp_src * random_tensor
            amp_src = amp_src.cuda()
        else:
            prop = offset
            amp_src = amp_src.squeeze(0).cpu()

...
class UnetFormerLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        if self.training and len(logits) == 2 and type(logits) is tuple:
    ...
    def forward(self, logits, labels):
        if self.training:
            if len(logits) == 2:
                ...
                bs = int(logits.size(0)/3)
                loss = (self.main_loss(logits[0:bs,:,:,:], labels)+self.main_loss(logits[bs:2*bs,:,:,:], labels)+self.main_loss(logits[2*bs:,:,:,:], labels))/3
        else:
            loss = self.main_loss(logits, labels)
        return loss
    '''

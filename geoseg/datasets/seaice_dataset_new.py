...
PALETTE = [[255, 255, 255], [255, 204, 0], [0, 0, 255], [0, 255, 128], [0, 204, 255], [0, 0, 0]]#[255, 255, 255],
...
class SeaIceDataset(Dataset):
    def __init__(self, data_root='data/seaice/test', mode='val', img_dir='image', mask_dir='mask',
                 img_suffix='.jpg', mask_suffix='.jpg', transform=val_aug, mosaic_ratio=0.0,
                 img_size=(512, 512)):
    ...
    def __getitem__(self, index):
        ...
        #####################################
        #print('aaaaaa', len(CLASSES))
        mask = (mask/(255/(len(CLASSES)-1))).type(torch.int64)
        #print(mask)
        #mask_new = torch.ones_like(mask)
        #mask = torch.where(mask > 4, mask_new, mask)
        #print(mask.dtype)
        #####################################
...
def show_img_mask_seg(seg_path, img_path, mask_path, start_seg_index):
    ...
    seg_list = [f for f in seg_list if f.endswith('.jpg')]
...
def show_mask(img, mask, img_id):
    ...
    ax2.imshow(mask)
    ax2.set_title('Mask ' + str(img_id)+'.jpg')

...
class Compose(object):
    ...
    def __call__(self, img1, img2, mask):
        assert img1.size == img2.size == mask.size
...
class RandomCrop(object):
    ...
    def __call__(self, img1, img2, mask, centroid=None):
        assert img1.size == img2.size == mask.size
        w, h = img1.size
        # ASSUME H, W
        th, tw = self.size
        ...
        if self.nopad:
            ...
            if pad_h or pad_w:
                img1 = ImageOps.expand(img1, border=border, fill=self.pad_color)
                img2 = ImageOps.expand(img2, border=border, fill=self.pad_color)
        ...
        return img1.crop((x1, y1, x1 + tw, y1 + th)), img2.crop((x1, y1, x1 + tw, y1 + th)), mask.crop((x1, y1, x1 + tw, y1 + th))
...
class PadImage(object):
    ...
    def __call__(self, img1, img2, mask):
        ...
        if w > tw or h > th:
            wpercent = (tw / float(w))
            target_h = int((float(img1.size[1]) * float(wpercent)))
            img1, img2, mask = img1.resize((tw, target_h), Image.BICUBIC), img2.resize((tw, target_h), Image.BICUBIC), mask.resize((tw, target_h), Image.NEAREST)

        w, h = img1.size
        img1 = ImageOps.expand(img1, border=(0, 0, tw - w, th - h), fill=0)
        img2 = ImageOps.expand(img2, border=(0, 0, tw - w, th - h), fill=0)
    ...
    def __call__(self, img1, img2, mask=None):
        if mask is not None:
            if random.random() < self.prob:
                return img1.transpose(Image.FLIP_LEFT_RIGHT), img2.transpose(Image.FLIP_LEFT_RIGHT), mask.transpose(Image.FLIP_LEFT_RIGHT)
            else:
                return img1, img2, mask
        else:
            if random.random() < self.prob:
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, img1, img2, mask=None):
        if mask is not None:
            if random.random() < self.prob:
                return img1.transpose(Image.FLIP_TOP_BOTTOM), img2.transpose(Image.FLIP_TOP_BOTTOM), mask.transpose(Image.FLIP_TOP_BOTTOM)
            else:
                return img1, img2, mask
        else:
            if random.random() < self.prob:
...
class RandomScale(object):
    ...
    def __call__(self, img1, img2, mask):
        ...
        h = int(scale_amt * oh)
        w = int(scale_amt * ow)
        return img1.resize((w, h), Image.BICUBIC), img2.resize((w, h), Image.BICUBIC), mask.resize((w, h), Image.NEAREST)
...
class SmartCropV1(object):
    ...
    def __call__(self, img1, img2, mask):
        ...
        count = 0
        while True:
            img1_crop, img2_crop, mask_crop = self.crop(img1.copy(), img2.copy(), mask.copy())
            count += 1
        ...
        return img1_crop, img2_crop, mask_crop
